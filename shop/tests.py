...
class BookStoreTest(TestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(
            ...
            stock=10,
            min_stock_limit=5,
            image="test_image.jpg"  # Shunchaki nom berib qo'yamiz
        )
    ...
    def test_search_product(self):
        ...
        # Home sahifasiga 'Ikki' so'zi bilan qidiruv beramiz
        response = self.client.get(reverse('home'), {'q': 'Ikki'})
        self.assertEqual(response.status_code, 200)
        # Qidiruv natijasida kitob nomi chiqishi kerak
    ...
    def test_stock_reduction_after_order(self):
        ...
        # Avvalgi stockni tekshiramiz (10 edi)
        self.assertEqual(self.product.stock, 10)

        # Buyurtma yaratamiz (2 ta kitob)
        ...
        # Mahsulotni qaytadan bazadan olib, stockini tekshiramiz
        self.product.refresh_from_db()
        self.assertEqual(self.product.stock, 8)  # 10 - 2 = 8 bo'lishi kerak



    def test_dashboard_access(self):
        """3. Xavfsizlik: Dashboardga faqat admin kira olishini tekshirish"""
        # Oddiy user kirmoqchi bo'lsa
        self.client.login(username='testuser', password='password123')
        response = self.client.get(reverse('dashboard'))
        # Admin bo'lmagani uchun redirect (302) qilishi kerak
    ...
    def test_order_creation_view(self):
        ...
        response = self.client.get(url)  # Kodingizda redirect bo'lgani uchun GET ishlatamiz
